<x-app-layout>
    <div class="max-w-4xl mx-auto">
        <div class="flex items-center mb-6">
            ...
            <h2 class="text-2xl font-bold text-gray-800">Editar Manutenção</h2>
        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="type" id="type" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full" required>
                            <option value="preventive" {{ $maintenance->type == 'preventive' ? 'selected' : '' }}>Preventiva</option>
                            <option value="corrective" {{ $maintenance->type == 'corrective' ? 'selected' : '' }}>Corretiva</option>
                        </select>
                    ...
                    <div>
                        ...
                        <x-text-input id="cost" class="block mt-1 w-full" type="number" step="0.01" name="cost" :value="old('cost', $maintenance->cost)" required />
                        <x-input-error :messages="$errors->get('cost')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="km_at_maintenance" :value="__('Quilometragem na Manutenção')" />
                        <x-text-input id="km_at_maintenance" class="block mt-1 w-full" type="number" name="km_at_maintenance" :value="old('km_at_maintenance', $maintenance->km_at_maintenance)" />
