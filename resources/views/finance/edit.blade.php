<x-app-layout>
    <div class="max-w-4xl mx-auto">
        <div class="flex items-center mb-6">
            ...
            <h2 class="text-2xl font-bold text-gray-800 dark:text-white">Editar Transação Financeira</h2>
        </div>

        <div class="bg-white dark:bg-slate-800 shadow-sm rounded-xl overflow-hidden border border-gray-100 dark:border-slate-700">
            <form action="{{ route('finance.update', $finance) }}" method="POST" class="p-8">
                @csrf
                @method('PATCH')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        <x-input-label for="description" :value="__('Descrição / Histórico')" />
                        <x-text-input id="description" class="block mt-1 w-full" type="text" name="description" :value="old('description', $finance->description)" required autofocus />
                    ...
                    <div>
                        ...
                        <select id="type" name="type" class="block mt-1 w-full border-gray-300 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 focus:border-blue-500 focus:ring-blue-500 rounded-md shadow-sm">
                            <option value="revenue" {{ $finance->type == 'revenue' ? 'selected' : '' }}>Receita (+)</option>
                            <option value="expense" {{ $finance->type == 'expense' ? 'selected' : '' }}>Despesa (-)</option>
                        </select>
                    ...
                    <div>
                        ...
                        <x-text-input id="amount" class="block mt-1 w-full" type="number" step="0.01" name="amount" :value="old('amount', $finance->amount)" required />
                        <x-input-error :messages="$errors->get('amount')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="date" :value="__('Data de Referência')" />
                        <x-text-input id="date" class="block mt-1 w-full" type="date" name="date" :value="old('date', \Carbon\Carbon::parse($finance->date)->format('Y-m-d'))" required />
                        <x-input-error :messages="$errors->get('date')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="due_date" :value="__('Data de Vencimento')" />
                        <x-text-input id="due_date" class="block mt-1 w-full" type="date" name="due_date" :value="old('due_date', $finance->due_date ? \Carbon\Carbon::parse($finance->due_date)->format('Y-m-d') : '')" />
                    ...
                    <div>
                        ...
                        <select id="category" name="category" class="block mt-1 w-full border-gray-300 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 focus:border-blue-500 focus:ring-blue-500 rounded-md shadow-sm">
                            @foreach(['Viagem','Manutenção','Combustível','Seguro','Salário','Outros'] as $cat)
                            <option value="{{ $cat }}" {{ $finance->category == $cat ? 'selected' : '' }}>{{ $cat }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="status" name="status" class="block mt-1 w-full border-gray-300 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 focus:border-blue-500 focus:ring-blue-500 rounded-md shadow-sm">
                            <option value="paid" {{ $finance->status == 'paid' ? 'selected' : '' }}>Pago / Recebido</option>
                            <option value="pending" {{ $finance->status == 'pending' ? 'selected' : '' }}>Pendente</option>
                        </select>
